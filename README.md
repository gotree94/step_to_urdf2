...
| 문제	 |  원인 | 
|:-------:|:-------:|
| 평면 법선(flat normal)만 저장	| STL은 각 삼각형 면 하나당 하나의 법선 벡터만 저장합니다. 인접한 삼각형끼리 법선이 공유되지 않아 면과 면 사이의 경계가 그대로 드러납니다 (faceted appearance). | 
| 곡면을 삼각형으로 근사	|  원래 CAD의 B-rep은 수학적 곡면(NURBS)입니다. STL로 변환하면 이를 삼각형으로 이산화(discretize) 하면서 deviation(허용 오차) 이하로 근사하는데, 이 근사 자체가 정보 손실입니다. | 
